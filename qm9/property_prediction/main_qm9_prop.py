...
def train(model, epoch, loader, mean, mad, property, device, partition='train', optimizer=None, lr_scheduler=None,
    ...
    for i, data in enumerate(loader):
        ...
        if partition == 'train':
            ...
            # label = label*27.2114
            # mad
            # loss = loss_l1(mad * pred *27.2114 + mean*27.2114, label*27.2114)
            # loss = loss_l1(pred, (label - mean) / mad)
...
def test(model, epoch, loader, mean, mad, property, device, log_interval, debug_break=False):
    return train(model, epoch, loader, mean, mad, property, device, partition='test', log_interval=log_interval,
...
if __name__ == "__main__":
    ...
    args.dataset = "qm9_second_half"  #de
    ...
    property_norms = utils.compute_mean_mad_from_dataloader(dataloaders['valid'].dataset, [args.property])
    ...
    for epoch in range(0, args.epochs):
        train(model, epoch, dataloaders['train'], mean, mad, args.property, device, partition='train',
              optimizer=optimizer, lr_scheduler=lr_scheduler, log_interval=args.log_interval)
        if epoch % args.test_interval == 0:
            val_loss = train(model, epoch, dataloaders['valid'], mean, mad, args.property, device, partition='valid',
                             optimizer=optimizer, lr_scheduler=lr_scheduler, log_interval=args.log_interval)
            test_loss = test(model, epoch, dataloaders['test'], mean, mad, args.property, device,
