...
from ..encoder import get_edge_encoder, EGNNSparseNetwork
from ..geometry import get_distance, eq_transform


class MGDMFullDP(nn.Module):
    ...
    def net(self, atom_type, pos, bond_index, bond_type, batch, time_step,
        ...
        if self.time_emb:
            ...
            # dynamic_embedding = DynamicTimestepEmbedding(self.config.hidden_dim).to(device)
            # temb = dynamic_embedding(time_step.to(device))
