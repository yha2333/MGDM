...
# from qm9.property_prediction.models import DistributionNodes
...
if __name__ == '__main__':
    ...
    parser.add_argument('--save_traj', action='store_true',
                        default="/apdcephfs/private_layneyhuang/MGDM/logs/model/checkpoint/drugs_default.pt",
    ...
    args.eta = 1
    args.global_start_sigma = 0.7  # float('inf')
    ...
    dataloader_train = DataLoader(train_set, 128, shuffle=False)
    ...
    diffusion_dataloader = DiffusionDataloader(args, model, nodes_dist, prop_dist,
                                               args.device, dataset_info, batch_size=args.batch_size,
    ...
    print("MGDM: We evaluate the classifier on our generated samples")
    loss = test(classifier, 799, diffusion_dataloader, mean, mad, args.context[0], args.device, 1, args.debug_break)
    print("Loss classifier on Generated samples: %.4f" % loss)
    logger.info('Loss classifier on Generated samples: %.4f' % loss)
    position_list = []
    atom_type_list = []
    mols_dict = []
