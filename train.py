...
def validate(it):
    if val_loader is not None:
        ...
        with torch.no_grad():
            ...
            # for batch in tqdm(it, desc='Validation'):
                batch = batch.to(device)
                if len(args.context) > 0:
...
if __name__ == '__main__':
    ...
    config_name = '%s_full_ddpm_2losses_2EGNN15_2attn+' % args.dataset  # 'qm9_full_temb_charge_norm_edmdataset' # log name
    ...
    if args.dataset == 'qm9':
        ...
        # val_set = Geom(pre_transform=transforms, split='valid')
        # val_loader = DataLoader(val_set, config.train.batch_size, shuffle=False)
        # 不创建验证数据加载器
        val_loader = None  # 确保没有验证数据加载器
    else:
    ...
    for it in range(start_iter, config.train.max_iters + 1):
        ...
        if it % config.train.val_freq == 0:
            ...
            # if avg_val_loss < best_val_loss:
                ...
                # 如果没有验证集，定期保存模型
                # if val_loader is None and it % config.train.save_freq == 0:
                if val_loader is None:
                    ckpt_path = os.path.join(ckpt_dir, '{}_iter_{}.pt'.format(args.dataset, it))
                    torch.save({
                        'config': config,
                        'model': model.state_dict(),
                        'optimizer_global': optimizer_global.state_dict(),
                    ...
                    print('Successfully saved the model at iteration {}!'.format(it))